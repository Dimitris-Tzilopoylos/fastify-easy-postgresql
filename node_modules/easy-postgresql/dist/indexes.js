    ...
    produceCreatePrimaryKeySQL() {
        ...
        if (auto_increment) {
            const sequence_name = `${DB.database}_${this.table}_${cols
                .map((x) => x.column)
                .join("_")}_sequence`;
            sequence = `CREATE SEQUENCE ${DB.database}.${sequence_name} INCREMENT 1 START 1 MINVALUE 1;ALTER TABLE ALTER COLUMN ${cols
                .map((x) => x.column)
                .join(",")} SET DEFAULT nextval('${sequence_name}'::regclass);`;
        }
        return `${sequence}ALTER TABLE ${DB.database}.${this.table} ADD PRIMARY KEY (${columns.map((x) => x.column)})`;
    }
