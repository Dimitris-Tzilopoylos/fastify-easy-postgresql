class QueryBuilder {
  ...
  select(columns = []) {
    ...
    if (!columns?.length) {
      ...
      config.parts = config.parts.concat(
        columns.map((x) => this.resolveColumnName(x, alias))
      );
    }
  ...
  where(...args) {
    ...
    const resolved = args.map((x) => this.resolveOperator(x, x?.type));
  ...
  resolveOperator(input, funcName) {
    ...
    if (!!this.bindingOperatorToSQL[funcName]) {
      const sql = input.args
        .map((x) => this.resolveOperator(x, x?.type))
        .join(` ${funcName} `);
      return `(${sql})`;
    }
...
const y = qb
  ...
  .from("users")
  .where(
    qb.eq(qb.column("id", "users"), 2),
    qb.and(
      qb.eq(qb.column("id", "users"), 2),
      qb.eq(qb.column("id", "users"), 2),
      qb.and(qb.eq(qb.column("id", "users"), qb.eq(qb.column("id", "users"))))
    )
  )
  .exec();
